...
import torch_scatter  # Add this import
from einops import rearrange, reduce
from flash_attn import flash_attn_func  # Import flash attention function
    ...
    def __init__(self, dim: int, mp_steps: int, dimensionality: int = 3):
        ...
        self.fused_message_update = nn.ModuleList([
            nn.Sequential(
                ...
                nn.GELU(),
                nn.LayerNorm(dim, elementwise_affine=False)  # Use non-affine LayerNorm for speed
...
class BallMSA(nn.Module):
    """ Ball Multi-Head Self-Attention (BMSA) module (eq. 8). Optimized version. """
    def __init__(self, dim: int, num_heads: int, ball_size: int, dimensionality: int = 3):
        ...
        # Fuse QKV projection into a single matrix for better performance
        self.qkv = nn.Linear(dim, 3 * dim, bias=False)  # Remove bias for speed
        self.proj = nn.Linear(dim, dim, bias=False)  # Remove bias for speed
        self.pe_proj = nn.Linear(dimensionality, dim, bias=False)  # Remove bias for speed
        self.sigma_att = nn.Parameter(-1 + 0.01 * torch.randn((1, num_heads, 1, 1)))

        # Pre-compute indices for reshaping operations
        self.register_buffer('reshape_indices', None, persistent=False)

    def _maybe_init_indices(self, batch_size):
        if self.reshape_indices is None or self.reshape_indices.size(0) != batch_size:
            self.reshape_indices = torch.arange(batch_size * self.ball_size, device=self.sigma_att.device)

    @torch.no_grad()
    def create_attention_mask(self, pos: torch.Tensor):
        """ Distance-based attention bias (eq. 10). Memory-efficient implementation. """
        pos = pos.view(-1, self.ball_size, pos.size(-1))
        # Use torch.cdist for better memory efficiency
        dist = torch.cdist(pos, pos, p=2, compute_mode='use_mm_for_euclid_dist')
        return self.sigma_att * dist.unsqueeze(1)

    @torch.no_grad()
    def compute_rel_pos(self, pos: torch.Tensor):
        """ Relative position of leafs wrt the center of the ball (eq. 9). Memory-efficient implementation. """
        pos = pos.view(-1, self.ball_size, pos.size(-1))
        mean = pos.mean(dim=1, keepdim=True)
        return (pos - mean).view(-1, pos.size(-1))

    def forward(self, x: torch.Tensor, pos: torch.Tensor):
        batch_size = x.size(0) // self.ball_size
        self._maybe_init_indices(batch_size)

        # Compute relative positions and add positional encoding
        rel_pos = self.compute_rel_pos(pos)
        x = x + self.pe_proj(rel_pos)

        # Compute QKV matrices in a single operation
        qkv = self.qkv(x)
        q, k, v = rearrange(qkv, "(n m) (t h d) -> t n h m d", t=3, h=self.num_heads, m=self.ball_size)

        # Compute attention bias from positions
        attn_bias = self.create_attention_mask(pos)

        # Reshape for flash attention
        q = rearrange(q, "n h m d -> (n h) m d")
        k = rearrange(k, "n h m d -> (n h) m d")
        v = rearrange(v, "n h m d -> (n h) m d")
        attn_bias = rearrange(attn_bias, "b h i j -> (b h) i j")

        # Use Flash Attention with pre-computed scale
        out = flash_attn_func(q, k, v, attn_bias, softmax_scale=self.scale, causal=False)
        
        # Reshape and project output
        out = rearrange(out, "(n h) m d -> n h m d", h=self.num_heads)
        x = rearrange(out, "n h m d -> (n m) (h d)")
        return self.proj(x)
